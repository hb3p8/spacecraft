...
void main()
{
	...
	sample = min(max(0.05, sample), 0.98);

	//final color
	color = vec4(texture(gradientTexture,vec2(sample,0.5)).xyz, 1-sqrt(sample));		
	//step is only required if you use mipmap or gradient texture with rgb!=0 at texCoord.t=1.0
}
